...
export function buildTrack(THREE, parent, table, opts) {
  ...
  const gauge = opts.gauge || 0.62;
  const railMat = mat(THREE, opts.rail || 0xd5dbe3, opts.railEmissive || 0x000000, 0.05);
  ...
  const railGeo = new THREE.BoxGeometry(0.09, 0.1, 1);
  ...
  for (let i = 0; i < n; i++) {
    ...
    if (supports && p.y > 2.4) {
      ...
      dummy.scale.set(0.38, Math.max(0.4, foot), 0.38);
      dummy.quaternion.identity();
      dummy.updateMatrix();
